...
export async function GET(
  ...
  try {
    ...
    console.log('Preview API parameters:', {
      format,
      includePersonalData,
      convertToEnglish,
      customHeaders,
    ...
    // CSVデータを生成（プレビュー用）
    try {
      console.log('Starting CSV generation with survey:', {
        id: survey.id,
      ...
      console.error('Error stack:', csvError instanceof Error ? csvError.stack : 'No stack trace')
      return NextResponse.json(
        { 
          message: 'Failed to generate CSV preview', 
          error: csvError instanceof Error ? csvError.message : 'Unknown error',
          details: csvError instanceof Error ? csvError.stack : undefined
        },
        { status: 500 }
      )
    }
...
function generateCSVData(survey: any, format: string, includePersonalData: boolean, convertToEnglish: boolean = false, customHeaders: any = null, variableNames: any = {}) {
  console.log('Preview generateCSVData called with:', { format, includePersonalData, convertToEnglish })
  
  try {
    const questions = survey.questions || []
    const responses = survey.responses || []
    
    console.log('Processing questions:', questions.length, 'responses:', responses.length)
  ...
  console.log('Preview Headers:', headers, 'customHeaders:', customHeaders)
    ...
    questions.forEach((question: any) => {
      ...
      if (format === 'raw') {
        ...
        if (['RADIO', 'SELECT', 'PREFECTURE', 'CHECKBOX'].includes(question.type)) {
          ...
          console.log(`OHE ${question.type} processing: format=${format}, numericValue=${numericValue}`)
          const processedValue = processNumericValue(numericValue, format, question, responses)
          console.log(`OHE ${question.type} processed: ${processedValue}`)
  ...
  console.log('CSV generation completed, rows:', rows.length)
  return rows.join('\n')
  } catch (error) {
    console.error('Error in generateCSVData:', error)
    throw error
  }
}
...
function getQuestionOptions(question: any): string[] {
  try {
    ...
    console.error('Error parsing question options:', error, 'question:', question.id)
    return []
  }
...
function convertToNumeric(question: any, answer: string): number {
  ...
  if (question.type === 'RATING') {
    ...
    console.log(`RATING conversion (preview): "${answer}" -> ${numericValue}`)
...
function processNumericValue(value: number, format: string, question: any, responses: any[]): number {
  console.log(`processNumericValue called: value=${value}, format=${format}, questionId=${question.id}`)
  
  // NAの値（0またはNaN）の場合はそのまま返す
  if (value === 0 || isNaN(value)) {
    console.log(`processNumericValue: returning original value (NA): ${value}`)
    return value
  }
  
  if (format === 'normalized' || format === 'standardized') {
    console.log(`processNumericValue: processing ${format} for question ${question.id}`)
    ...
    console.log(`Statistics: mean=${mean}, stdDev=${stdDev}, numericValues=[${numericValues.join(',')}]`)
    
    if (format === 'normalized') {
      // 正規化: (x - min) / (max - min)
      const min = Math.min(...numericValues)
      const max = Math.max(...numericValues)
      console.log(`Normalization: min=${min}, max=${max}, value=${value}`)
      if (max === min) return 0
      ...
      // 標準化: (x - mean) / std
      console.log(`Standardization: mean=${mean}, stdDev=${stdDev}, value=${value}`)
      if (stdDev === 0) return 0
      const result = (value - mean) / stdDev
      console.log(`Standardized result: ${result}`)
      return result
    }
  }
